...
public class UFE : MonoBehaviour {
	...
	public static void JoinBluetoothGame(){
		...
		if (UFE.isNetworkAddonInstalled){
			...
			UFE.multiplayerAPI.FindLanGames();
		}
		#endif
	}
	...
	protected static void OnLanGamesDiscovered(ReadOnlyCollection<string> addresses){
		...
		if (addresses != null && addresses.Count > 0){
			...
			UFE.multiplayerAPI.Connect(addresses[0], UFE.config.networkOptions.port);
		}else{
	...
	#if !UNITY_WEBGL
	protected static void OnLanGamesDiscoveryError(NetworkConnectionError error){
		UFE.multiplayerAPI.OnLanGamesDiscovered -= UFE.OnLanGamesDiscovered;
		UFE.multiplayerAPI.OnLanGamesDiscoveryError -= UFE.OnLanGamesDiscoveryError;
		UFE.StartConnectionLostScreen();
    }
	...
	protected void Awake(){
        ...
        #if !UNITY_WEBGL
		string uuid = UFE.config.gameName ?? "UFE";
		if (UFE.isNetworkAddonInstalled){
			...
			UFE.multiplayerAPI.SendRate = 1f / (float)UFE.config.fps;
		...
		if (UFE.isAiAddonInstalled){
            ...
            p1Controller.cpuController = gameObject.AddComponent(UFE.SearchClass("RuleBasedAI2")) as AbstractInputController;
        }
        else{
		...
		if (UFE.isAiAddonInstalled){
            ...
            p2Controller.cpuController = gameObject.AddComponent(UFE.SearchClass("RuleBasedAI2")) as AbstractInputController;
        }
        else{
...
//			);
	...
	protected static void AddNetworkEventListeners(){
		UFE.multiplayerAPI.OnServerConnectionSuccess -= UFE.OnConnectedToServer;
		UFE.multiplayerAPI.OnDisconnection -= UFE.OnDisconnectedFromServer;
		UFE.multiplayerAPI.OnServerConnectionError -= UFE.OnFailedToConnect;
		UFE.multiplayerAPI.OnPlayerConnectedToServer -= UFE.OnPlayerConnected;
		UFE.multiplayerAPI.OnPlayerDisconnectedFromServer -= UFE.OnPlayerDisconnected;
		UFE.multiplayerAPI.OnServerStarted -= UFE.OnServerInitialized;
//		UFE.multiplayerAPI.OnLanGamesDiscovered -= UFE.OnLanGamesDiscovered;
//		UFE.multiplayerAPI.OnLanGamesDiscoveryError -= UFE.OnLanGamesDiscoveryError;
...
//		UFE.multiplayerAPI.OnLanGamesDiscoveryError -= UFE.OnLanGamesDiscoveryError;
	...
	protected static void OnDisconnectedFromServer(NetworkDisconnection info) {
        if (UFE.config.debugOptions.connectionLog) Debug.Log("Disconnected from server: " + info);
	...
	protected static void OnFailedToConnect(NetworkConnectionError error) {
        if (UFE.config.debugOptions.connectionLog) Debug.Log("Could not connect to server: " + error);
		Application.runInBackground = UFE.config.runInBackground;
		UFE.StartConnectionLostScreen();
	}

	protected static void OnPlayerConnected(NetworkPlayer player) {
        if (UFE.config.debugOptions.connectionLog) Debug.Log("Player connected from " + player.ipAddress + ":" + player.port);
		UFE.StartNetworkGame(0.5f, 1);
	}

	protected static void OnPlayerDisconnected(NetworkPlayer player) {
	...
	protected static void OnServerInitialized() {
		...
		if (UFE.config.networkOptions.fakeNetwork){
			UFE.OnPlayerConnected(UFE.multiplayerAPI.NetworkPlayer);
		}
	}
	#endif
	#endregion
