            ...
            if (updateConfirm) {
                ...
                foreach (CharacterInfo character in globalInfo.characters) {
                    ...
                    foreach (MoveSetData moveSetData in character.moves) {
                        ...
                        basicMoveUpdate(moveSetData.basicMoves.getHitMidKnockdown, WrapMode.ClampForever, false);
                        ...
                        basicMoveUpdate(moveSetData.basicMoves.groundBounce, WrapMode.ClampForever, false);
                        basicMoveUpdate(moveSetData.basicMoves.airWallBounce, WrapMode.ClampForever, false);
                        basicMoveUpdate(moveSetData.basicMoves.fallingFromGroundBounce, WrapMode.ClampForever, true);
                ...
                if (bounceOptions) {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        EditorGUI.BeginDisabledGroup(globalInfo.groundBounceOptions.bounceForce == Sizes.None);
                        {
                            ...
                            globalInfo.groundBounceOptions.minimumBounceForce = EditorGUILayout.FloatField("Minimum Bounce Force:", globalInfo.groundBounceOptions.minimumBounceForce);
                            globalInfo.groundBounceOptions.maximumBounces = EditorGUILayout.FloatField("Maximum Bounces:", globalInfo.groundBounceOptions.maximumBounces);
                            ...
                            if (globalInfo.groundBounceOptions.shakeCamOnBounce) {
                                globalInfo.groundBounceOptions.shakeDensity = EditorGUILayout.FloatField("Shake Density:", globalInfo.groundBounceOptions.shakeDensity);
                            }
                        }
                        EditorGUI.EndDisabledGroup();
                        ...
                        EditorGUI.BeginDisabledGroup(globalInfo.wallBounceOptions.bounceForce == Sizes.None);
                        {
                            ...
                            globalInfo.wallBounceOptions.minimumBounceForce = EditorGUILayout.FloatField("Minimum Bounce Force:", globalInfo.wallBounceOptions.minimumBounceForce);
                            globalInfo.wallBounceOptions.maximumBounces = EditorGUILayout.FloatField("Maximum Bounces:", globalInfo.wallBounceOptions.maximumBounces);
                            ...
                            if (globalInfo.wallBounceOptions.shakeCamOnBounce) {
                                globalInfo.wallBounceOptions.shakeDensity = EditorGUILayout.FloatField("Shake Density:", globalInfo.wallBounceOptions.shakeDensity);
                            }
                        }
                        EditorGUI.EndDisabledGroup();
			...
			EditorGUILayout.BeginVertical(rootGroupStyle);{
				...
				if (screenOptions){
					EditorGUILayout.BeginVertical(subGroupStyle);{
                        ...
                        globalInfo.gameGUI.screenFadeDuration = Mathf.Max(0f, EditorGUILayout.FloatField("Screen Fade-in Duration:", globalInfo.gameGUI.screenFadeDuration));
                        globalInfo.gameGUI.screenFadeColor = EditorGUILayout.ColorField("Screen Fade-in Color:", globalInfo.gameGUI.screenFadeColor);
                        globalInfo.gameGUI.gameFadeDuration = Mathf.Max(0f, EditorGUILayout.FloatField("Game Fade-in Duration:", globalInfo.gameGUI.gameFadeDuration));
                        ...
                        if (guiScreensOptions) {
							...
							EditorGUILayout.BeginVertical(this.subGroupStyle);{
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    globalInfo.gameGUI.skillTreeScreen = (SkillTreeScreen)EditorGUILayout.ObjectField("Skill Tree:", globalInfo.gameGUI.skillTreeScreen, typeof(SkillTreeScreen), true);
                                    EditorGUI.BeginDisabledGroup(DisableScreenButton(globalInfo.gameGUI.optionsScreen));
                                    {
                                        ScreenButton(globalInfo.gameGUI.skillTreeScreen);
                                    }
                                    EditorGUI.EndDisabledGroup();
                                }
                                EditorGUILayout.EndHorizontal();
								...
								if (UFE.isNetworkAddonInstalled){
									...
									/*EditorGUILayout.BeginHorizontal();{
										globalInfo.gameGUI.bluetoothGameScreen = (BluetoothGameScreen)EditorGUILayout.ObjectField("Bluetooth Game:", globalInfo.gameGUI.bluetoothGameScreen, typeof(BluetoothGameScreen), true);
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                ...
                if (preloadOptions) {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        EditorGUI.indentLevel += 1;
                        EditorGUIUtility.labelWidth = 200;
                        globalInfo.preloadingTime = EditorGUILayout.FloatField("Preloading Time:", globalInfo.preloadingTime);
				...
				EditorGUILayout.BeginVertical(rootGroupStyle);{
					...
					if (networkOptions){
						EditorGUILayout.BeginVertical(subGroupStyle);{
							...
							globalInfo.networkOptions.disconnectOnDesync = EditorGUILayout.Toggle("Disconnect on Desync", globalInfo.networkOptions.disconnectOnDesync);

                            EditorGUILayout.Space();
                            SubGroupTitle("Service");
                            ...
                            if (globalInfo.networkOptions.networkService == NetworkService.P2P) {
                                globalInfo.networkOptions.port = EditorGUILayout.IntField("Network Port:", globalInfo.networkOptions.port);

                            /*} else if (globalInfo.networkOptions.networkService == NetworkService.Playfab) {
                                globalInfo.networkOptions.titleId = EditorGUILayout.TextField("Title Id:", globalInfo.networkOptions.titleId);
                                globalInfo.networkOptions.photonKey = EditorGUILayout.TextField("Photon Application ID:", globalInfo.networkOptions.photonKey);

                            } else if (globalInfo.networkOptions.networkService == NetworkService.UFEOnline) {
                                globalInfo.networkOptions.authKey = EditorGUILayout.TextField("Auth Key:", globalInfo.networkOptions.authKey);
                                globalInfo.networkOptions.titleId = EditorGUILayout.TextField("Title Id:", globalInfo.networkOptions.titleId);
                            ...
                            */} else {
                            ...
                            /*
                            if (globalInfo.networkOptions.networkService != NetworkService.P2P) {
                                EditorGUI.BeginDisabledGroup(globalInfo.networkOptions.networkService == NetworkService.Unity);{

                                    lobbyOptions = EditorGUILayout.Foldout(lobbyOptions, "Lobbies (" + globalInfo.networkOptions.lobbies.Length + ")", foldStyle);
                                    if (lobbyOptions) {
                                        EditorGUILayout.BeginVertical(subGroupStyle);{
                                            EditorGUILayout.Space();

                                            EditorGUIUtility.labelWidth = 170;
                                            for (int i = 0; i < globalInfo.networkOptions.lobbies.Length; i++) {
                                                EditorGUILayout.Space();
                                                EditorGUILayout.BeginVertical(arrayElementStyle);{
                                                    EditorGUILayout.Space();
                                                    EditorGUILayout.BeginHorizontal();{
                                                        globalInfo.networkOptions.lobbies[i].lobbyName = EditorGUILayout.TextField("Name:", globalInfo.networkOptions.lobbies[i].lobbyName);
                                                        if (GUILayout.Button("", "PaneOptions")) {
                                                            PaneOptions<LobbyOptions>(globalInfo.networkOptions.lobbies, globalInfo.networkOptions.lobbies[i], delegate(LobbyOptions[] newElement) { globalInfo.networkOptions.lobbies = newElement; });
                                                        }
                                                    }EditorGUILayout.EndHorizontal();

                                                    globalInfo.networkOptions.lobbies[i].matchMakingType = (LobbyMatchCreationSystem)EditorGUILayout.EnumPopup("Match Type:", globalInfo.networkOptions.lobbies[i].matchMakingType, enumStyle);
                                                    globalInfo.networkOptions.lobbies[i].gameMode = (GameMode)EditorGUILayout.EnumPopup("Game Mode:", globalInfo.networkOptions.lobbies[i].gameMode, enumStyle);
                                                    if (globalInfo.networkOptions.lobbies[i].matchMakingType == LobbyMatchCreationSystem.ManualRoomCreation) {
                                                        globalInfo.networkOptions.lobbies[i].allowPrivateRooms = EditorGUILayout.Toggle("Allow Private Rooms", globalInfo.networkOptions.lobbies[i].allowPrivateRooms);
                                                    } else {
                                                        NetworkUserDataBlock("Match Making Data Filter", globalInfo.networkOptions.lobbies[i].matchMakingUserData, "filter", ref globalInfo.networkOptions.lobbies[i].matchMakingToggle, delegate(NetworkUserData[] obj){ globalInfo.networkOptions.lobbies[i].matchMakingUserData = obj; });
                                                    }

                                                    NetworkUserDataBlock("Winner Data Update", globalInfo.networkOptions.lobbies[i].winnerUserData, "update", ref globalInfo.networkOptions.lobbies[i].winnerToggle, delegate(NetworkUserData[] obj){ globalInfo.networkOptions.lobbies[i].winnerUserData = obj; });
                                                    NetworkUserDataBlock("Loser Data Update", globalInfo.networkOptions.lobbies[i].loserUserData, "update", ref globalInfo.networkOptions.lobbies[i].loserToggle, delegate(NetworkUserData[] obj) { globalInfo.networkOptions.lobbies[i].loserUserData = obj; });

                                                }EditorGUILayout.EndVertical();
                                            }

                                            EditorGUILayout.Space();
                                            if (StyledButton("New Lobby"))
                                                globalInfo.networkOptions.lobbies = AddElement<LobbyOptions>(globalInfo.networkOptions.lobbies, new LobbyOptions());
                            ...
                            */
                            ...
                            EditorGUILayout.Space();
                            SubGroupTitle("Netcode");
                            //globalInfo.networkOptions.automaticFrameDelay = EditorGUILayout.Toggle("Automatic Frame Delay", globalInfo.networkOptions.automaticFrameDelay);
                            EditorGUI.BeginDisabledGroup(globalInfo.networkOptions.automaticFrameDelay);{
                                globalInfo.networkOptions.defaultFrameDelay = EditorGUILayout.IntField("Min Frame Delay: ", Mathf.Clamp(globalInfo.networkOptions.defaultFrameDelay, 0, globalInfo.networkOptions.maxFrameDelay));
                            }EditorGUI.EndDisabledGroup();
                            globalInfo.networkOptions.maxFrameDelay = EditorGUILayout.IntField("Max Frame Delay: ", globalInfo.networkOptions.maxFrameDelay);
                            
                            /*
                            EditorGUILayout.Space();
                            globalInfo.networkOptions.userRollBack = EditorGUILayout.Toggle("Use Rollback", globalInfo.networkOptions.userRollBack);
                            globalInfo.networkOptions.rollBackFrames = EditorGUILayout.IntField("Max Rollback Frames: ", globalInfo.networkOptions.rollBackFrames);
                            */
