...
public class RuleBasedAI2 : RandomAI{
    ...
    {
        if (UFE.GetLocalPlayer() == UFE.GetPlayer1().playerNum)
            base.DoFixedUpdate();
        else
        {
            if (nameScript.gameMode.Contains("Fuzzy"))
                this.DoFixedUpdateRuleBased();
            else
                base.DoFixedUpdate();
        }
    }

    public void DoFixedUpdateRuleBased(){
		// Store initial AI
    ...
    {
        if (UFE.GetLocalPlayer() == UFE.GetPlayer1().playerNum)
            this.DoUpdateDecision();
        else
        {
            if (nameScript.gameMode.Contains("Fuzzy"))
                this.DoUpdateRuleBased();
            else
                this.DoUpdateDecision();
        }
    }
    ...
    {
        if (UFE.GetLocalPlayer() == UFE.GetPlayer1().playerNum)
            return this.ReadInputDecision(inputReference);
        else
            return (nameScript.gameMode.Contains("Fuzzy") ? this.ReadInputRuleBased(inputReference) : this.ReadInputDecision(inputReference));
    }

    public InputEvents ReadInputRuleBased (InputReferences inputReference){
			...
			for (int j = 0; j < move.attackMoves.Length; ++j){
				...
				if (moveInfo != null && self.MoveSet.ValidateMoveExecution(moveInfo)){
					...
					if (ai.advancedOptions.reactionParameters.enableGaugeFilter){
						...
						gaugeUsageWeight += gaugeVar.GetLabelMembership(RuleBasedAI2.gaugeUsageNone, gaugeUsage) * gaugeUsageNone;
						gaugeUsageWeight += gaugeVar.GetLabelMembership(RuleBasedAI2.gaugeUsageQuarter, gaugeUsage) * gaugeUsageQuarter;
						gaugeUsageWeight += gaugeVar.GetLabelMembership(RuleBasedAI2.gaugeUsageHalf, gaugeUsage) * gaugeUsageHalf;
						gaugeUsageWeight += gaugeVar.GetLabelMembership(RuleBasedAI2.gaugeUsageThreeQuarters, gaugeUsage) * gaugeUsageThreeQuarters;
						gaugeUsageWeight += gaugeVar.GetLabelMembership(RuleBasedAI2.gaugeUsageAll, gaugeUsage) * gaugeUsageAll;
						++count;
					}
					...
					if (moveInfo.hits.Length > 0){
						if (ai.advancedOptions.reactionParameters.enableDamageFilter){
							...
							damageWeight += damageVar.GetLabelMembership(RuleBasedAI2.damageVeryWeak, damage) * damageVeryWeak;
							damageWeight += damageVar.GetLabelMembership(RuleBasedAI2.damageWeak, damage) * damageWeak;
							damageWeight += damageVar.GetLabelMembership(RuleBasedAI2.damageMedium, damage) * damageMedium;
							damageWeight += damageVar.GetLabelMembership(RuleBasedAI2.damageStrong, damage) * damageStrong;
							damageWeight += damageVar.GetLabelMembership(RuleBasedAI2.damageVeryStrong, damage) * damageVeryStrong;
							++count;
						}
	...
	protected virtual void OnGameBegin(CharacterInfo player1, CharacterInfo player2, StageOptions stage){
		...
		if (self != null){
			...
			//int count;
			float deltaTime = 1f / (float)(UFE.config.fps);
			//int frames = Mathf.FloorToInt(this.ai.advancedOptions.timeBetweenDecisions / deltaTime) + 1;
			int frames = Mathf.FloorToInt(this.ai.advancedOptions.movementDuration * (float)UFE.config.fps);
			...
			int framesAfterAttack = Mathf.FloorToInt(newTimeBetweenActions * (float)UFE.config.fps);

			//int jumpDelay = self.myInfo.physics.jumpDelay;
			int jumpDelay = 1;

			int paddingAfterJump = Mathf.Max(UFE.config.plinkingDelay + 1, Mathf.FloorToInt(self.myInfo.executionTiming / deltaTime) + 1);
			...
			// 1) Then we enter the horizontal input
			jumpBackward.Add(new ButtonPress[]{ButtonPress.Back});
			jumpForward.Add(new ButtonPress[]{ButtonPress.Foward});
			
			
			// 2) Finally we enter the vertical input...
			jumpBackward.AddRange(this.Repeat(new ButtonPress[]{ButtonPress.Up}, jumpDelay));
			jumpForward.AddRange(this.Repeat(new ButtonPress[]{ButtonPress.Up}, jumpDelay));
			jumpStraight.AddRange(this.Repeat(new ButtonPress[]{ButtonPress.Up}, jumpDelay));
			...
			this.moveForwardSimulatedInput = this.Repeat(new ButtonPress[]{ButtonPress.Foward}, frames);
			...
			for (int i = 0; i < self.myInfo.moves.Length; ++i){
				...
				for (int j = 0; j < move.attackMoves.Length; ++j){
					...
					if (moveInfo != null){
						...
						for (int k = 0; k < moveInfo.buttonSequence.Length; ++k){
							ButtonPress currentInput = moveInfo.buttonSequence[k];
    ...
    public override void Initialize(IEnumerable<InputReferences> inputs, int bufferSize)
    {
        ...
        dta.LoadJSON("./C45algorithm-master/nikkifox.json");
    ...
    public InputEvents ReadInputDecision(InputReferences inputReference)
    {
        ControlsScript self = UFE.GetControlsScript(this.player);
        if (self != null)
        {
